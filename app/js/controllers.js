...
weatherControllers.controller('CityListCtrl', ['$rootScope', '$scope', '$http', '$location',
  function($rootScope, $scope, $http, $location) {
    ...
    $scope.add_city = function(id){
      ...
      if (!dup){
        var url = 'http://api.openweathermap.org/data/2.5/weather?id=' + id + '&units=metric&APPID=4b54b60fe793028adf7488a46179a1d1'
...
weatherControllers.controller('CityDetailCtrl', ['$scope', '$routeParams', '$http', 
  function($scope, $routeParams, $http) {
    var url = 'http://api.openweathermap.org/data/2.5/forecast/daily?id=' + $routeParams.id + '&cnt=6&units=metric&APPID=4b54b60fe793028adf7488a46179a1d1'
    $http.get(url).success( function(data){
      $scope.forecast = data;
